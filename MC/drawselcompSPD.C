void drawselcompSPD(int imc=0, const char* histo="gMultXiS", const char* histo2="gZDCXiS"){
  int multSel[2] = {1,4};
  int eeSel[2] = {1,4};
  TFile *f = new TFile(Form("mctruth%d_SPD.root",imc));
  TGraphErrors *g = (TGraphErrors *) f->Get(histo);
  TGraphErrors *gbis = (TGraphErrors *) f->Get(histo2);

  TFile *f2 = new TFile(Form("mctruthSD%d_SPD.root",imc));
  ...
  TFile *fout = new TFile(Form("comparison%dSPD.root",imc), "RECREATE");
