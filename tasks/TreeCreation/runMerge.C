  ...
  
{
  ...
  if(Period==122) ms->SetAlternateOADBFullManualBypass("alien:///alice/cern.ch/user/f/fercoles/DataFiles/OADB-LHC12b-MB.root");
  ...
  if (grid)
	...
	for (Int_t i= 0;i <26; i++) alienHandler->AddRunNumber(runList[i]);
	
	alienHandler->SetGridWorkingDir("AnalysisLeading2019/TreeGeneration/12bpass2/");
      }
